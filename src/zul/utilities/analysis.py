...
class ChartGenerator:
    ...
    def _create_comparison_bar_chart(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        labels = config.get("labels", [f"Item {i+1}" for i in range(len(data[datasets[0]]))])
        ...
        for i, (dataset_name, dataset_values) in enumerate(data.items()):
            ...
            bar = self.current_ax.bar(x + offset, dataset_values, width, 
            ...
            for j, (bar_rect, value) in enumerate(zip(bar, dataset_values)):
                self.current_ax.annotate(f'{value:.1f}', 
                ...
                if i < len(baseline_data):
                    ...
                    for j, (dataset_name, dataset_values) in enumerate(data.items()):
                        if j > 0 and i < len(dataset_values) and baseline_val != 0:
                            diff_pct = ((dataset_values[i] - baseline_val) / baseline_val) * 100
                            diff_text.append(f"{dataset_name}: {diff_pct:+.1f}%")
                    
                    if diff_text:
                        self.current_ax.text(i, y_pos, '\n'.join(diff_text), 
    ...
    def _create_comparison_line_chart(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i, (dataset_name, dataset_values) in enumerate(data.items()):
            ...
            self.current_ax.plot(labels, dataset_values, label=dataset_name, 
                               color=colors[i], marker=marker, linewidth=2, 
                               markersize=6, alpha=self.config["alpha"])
            ...
            if show_statistics:
                ...
                self.current_ax.axhline(y=mean_val, color=colors[i], linestyle='--', 
                                      alpha=0.6, label=f'{dataset_name} Mean: {mean_val:.2f}')
    ...
    def _create_comparison_box_chart(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for dataset_name, dataset_values in data.items():
            stats_summary[dataset_name] = {
                ...
                'iqr': np.percentile(dataset_values, 75) - np.percentile(dataset_values, 25),
                'min': np.min(dataset_values),
                'max': np.max(dataset_values)
            }
        ...
        if len(datasets) >= 2:
            ...
            if show_statistics:
                ...
                self.current_ax.text(0.02, 0.98, stats_text, transform=self.current_ax.transAxes,
    ...
    def _create_comparison_histogram(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if overlay:
            ...
            for i, (dataset_name, dataset_values) in enumerate(data.items()):
                self.current_ax.hist(dataset_values, bins=bins, alpha=self.config["alpha"], 
                                   label=dataset_name, color=colors[i], density=density)
            ...
            n_datasets = len(datasets)
            self.current_fig, axes = plt.subplots(n_datasets, 1, figsize=(figsize[0], figsize[1] * n_datasets))
            if n_datasets == 1:
                axes = [axes]
            
            for i, (dataset_name, dataset_values) in enumerate(data.items()):
                axes[i].hist(dataset_values, bins=bins, color=colors[i],  # type: ignore
                           alpha=self.config["alpha"], density=density)
    ...
    def _create_comparison_dashboard(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i, (dataset_name, dataset_values) in enumerate(data.items()):
            offset = (i - n_datasets/2 + 0.5) * width
            ax1.bar([j + offset for j in x], dataset_values[:len(x)], width, 
                   label=dataset_name, alpha=self.config["alpha"], color=colors[i])
        ...
        bp = ax2.boxplot([data[ds] for ds in datasets], labels=datasets, patch_artist=True)
        ...
        for i, (dataset_name, dataset_values) in enumerate(data.items()):
            ...
            ax3.plot(x_range[:len(dataset_values)], dataset_values, 'o-', 
                    label=dataset_name, color=colors[i], linewidth=2, markersize=4)
            ax3.axhline(y=np.mean(dataset_values), color=colors[i], linestyle='--', alpha=0.6)
        ...
        for i, (dataset_name, dataset_values) in enumerate(data.items()):
            ax4.hist(dataset_values, bins=15, alpha=0.6, label=dataset_name, 
        ...
        if len(datasets) >= 2:
            stats_summary['statistical_tests'] = self._perform_statistical_tests(data)
            stats_summary['performance_comparisons'] = self._calculate_performance_comparisons(data)
        
        # Format statistics text
        stats_text = self._format_comprehensive_statistics_text(stats_summary, datasets, metric_name)
    ...
    def _perform_statistical_tests(self, data: Dict[str, List[float]]) -> Dict[str, Any]:
    ...
    def _calculate_performance_comparisons(self, data: Dict[str, List[float]]) -> Dict[str, Any]:
    ...
    def _format_statistics_text(self, stats_summary: Dict[str, Any]) -> str:
        ...
        if 'statistical_tests' in stats_summary:
            ...
            if 't_test' in tests:
                ...
                text_lines.append(f"Significant: {'Yes' if tests['t_test']['significant'] else 'No'}")
            
            if 'anova' in tests:
                text_lines.append(f"ANOVA p-value: {tests['anova']['p_value']:.4f}")
                text_lines.append(f"Significant: {'Yes' if tests['anova']['significant'] else 'No'}")
        
        return '\n'.join(text_lines)
    
    def _format_comprehensive_statistics_text(self, stats_summary: Dict[str, Any], 
                                            datasets: List[str], metric_name: str) -> str:
        ...
        if 'performance_comparisons' in stats_summary:
            ...
            for comparison_name, comparison_data in stats_summary['performance_comparisons'].items():
...
def create_chart_from_csv(chart_gen: ChartGenerator, csv_file: str, config: Dict[str, Any]) -> Dict[str, Any]:
    """
...
def batch_create_charts(chart_gen: ChartGenerator, configs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
